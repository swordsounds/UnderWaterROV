...
class MotorPositionController(Phidget):
	...
	def getFailsafeBrakingStrength(self):
		_FailsafeBrakingStrength = ctypes.c_double()

		__func = PhidgetSupport.getDll().PhidgetMotorPositionController_getFailsafeBrakingStrength
		__func.restype = ctypes.c_int32
		result = __func(self.handle, ctypes.byref(_FailsafeBrakingStrength))

		if result > 0:
			raise PhidgetException(result)

		return _FailsafeBrakingStrength.value

	def setFailsafeBrakingStrength(self, FailsafeBrakingStrength):
		_FailsafeBrakingStrength = ctypes.c_double(FailsafeBrakingStrength)

		__func = PhidgetSupport.getDll().PhidgetMotorPositionController_setFailsafeBrakingStrength
		__func.restype = ctypes.c_int32
		result = __func(self.handle, _FailsafeBrakingStrength)
